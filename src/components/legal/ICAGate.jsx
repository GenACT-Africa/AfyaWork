/**
 * ICAGate — blocks CO access until the Independent Contractor Agreement is signed.
 *
 * Wraps all CO routes. On mount it checks co_profiles.ica_signed_at.
 * If null → shows a full-screen modal with the PDF and a sign button.
 * Once signed → renders children normally.
 */
...
export default function ICAGate({ children }) {
  ...
  useEffect(() => {
    if (!user?.id) return;
    getCOICAStatus(user.id).then(({ signed }) => {
      setStatus(signed ? 'signed' : 'needed');
    });
  }, [user?.id]);
  ...
  // Still loading or already signed — transparent passthrough
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 shrink-0">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Please read the full agreement before signing</p>
            </div>
          </div>
      ...
      {/* Render children underneath (blurred) so layout doesn't break */}
