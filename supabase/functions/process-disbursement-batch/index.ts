 ...
 *
 * MOCK MODE (for testing without Selcom credentials):
 *   Set system_config key 'selcom_mock_mode' = 'true'  — OR —
 *   leave SELCOM_API_KEY env var unset.
 *   In mock mode all payments are immediately marked as 'disbursed'
 *   with a MOCK-* transaction reference. No real money moves.
 *
 * Required env vars:
 *   SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY  — auto-injected
 *   SELCOM_API_KEY     — Selcom API key       (optional in mock mode)
 *   SELCOM_API_SECRET  — Selcom API secret    (optional in mock mode)
 *   SELCOM_WEBHOOK_URL — selcom-webhook function URL (optional in mock mode)
 */
...
// ⚠️  Update per official Selcom documentation.
const SELCOM_BASE = {
...
// ── Mock disbursement (no real API call) ──────────────────────────
// Returns a successful result instantly. Safe for testing.
function mockDisbursement(reference: string): SelcomTransferResult {
  return {
    success:             true,
    transactionRef:      `MOCK-${reference.slice(0, 12).toUpperCase()}`,
    responseCode:        'MOCK_SUCCESS',
    responseDescription: 'Mock disbursement — no real payment made. Replace with Selcom credentials for live payments.',
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: configs } = await db
      .from('system_config')
      .select('key, value')
      .in('key', ['selcom_environment', 'selcom_status', 'selcom_mock_mode']);

    const cfgMap = Object.fromEntries((configs || []).map((c: any) => [c.key, c.value]));

    if (cfgMap.selcom_status === 'paused') {
      return new Response(JSON.stringify({ ok: false, reason: 'Selcom disbursements are paused by admin' }), {
        headers: { ...CORS, 'Content-Type': 'application/json' },
      });
    }

    // Mock mode: explicit config flag OR no API credentials present
    const isMockMode = cfgMap.selcom_mock_mode === 'true' || !SELCOM_API_KEY;
    ...
    const { data: batch, error: batchErr } = await db
      ...
      .insert({
        ...
        selcom_batch_reference: isMockMode ? 'MOCK-BATCH' : null,
      })
      .select('id')
      .single();
    ...
    if (!payments || payments.length === 0) {
      await db.from('disbursement_batches').update({
        ...
        total_cos_paid: 0, total_shifts_covered: 0, total_amount_disbursed: 0,
      }).eq('id', batchId);

      return new Response(JSON.stringify({ ok: true, message: 'No scheduled payments', batch_id: batchId, mock: isMockMode }), {
        headers: { ...CORS, 'Content-Type': 'application/json' },
      });
    }

    // ── Group by CO ───────────────────────────────────────────────
    type CoGroup = { coId: string; provider: string; number: string; total: number; paymentIds: string[]; shiftIds: string[] };
    ...
    for (const p of payments as any[]) {
      ...
      if (!byCoMap[p.co_id]) {
        byCoMap[p.co_id] = { coId: p.co_id, provider: p.mobile_money_provider, number: p.mobile_money_number, total: 0, paymentIds: [], shiftIds: [] };
      }
      byCoMap[p.co_id].total += p.adjusted_pay_amount ?? p.co_total_pay;
    ...
    let totalCOsPaid = 0, totalDisbursed = 0, totalFailed = 0, totalProcessing = 0;
    ...
    for (const grp of coGroups) {
      ...
      // ── MOCK PATH ─────────────────────────────────────────────
      if (isMockMode) {
        const mockResult = mockDisbursement(reference);

        // In mock mode, mark disbursed immediately (no webhook needed)
        await db.from('shift_payments').update({
          payment_status:              'disbursed',
          disbursement_batch_id:       batchId,
          selcom_transaction_ref:      mockResult.transactionRef,
          selcom_response_code:        mockResult.responseCode,
          selcom_response_description: mockResult.responseDescription,
          selcom_raw_response:         mockResult.rawResponse,
          selcom_webhook_received_at:  new Date().toISOString(), // simulated
        ...
        // Still fire WA notifications (fire-and-forget)
      ...
      await db.from('shift_payments').update({
        ...
        disbursement_batch_id: batchId,
        selcom_transaction_ref: reference,
      }).in('id', grp.paymentIds);

      const result = await callSelcomDisbursement(baseUrl, reference, grp.number, grp.provider, grp.total, description);
      ...
      if (result.success) {
        ...
        if (providerStats[grp.provider]) providerStats[grp.provider].failed += shiftCount;
    ...
    // ── Finalise batch ────────────────────────────────────────────
    const batchStatus = totalFailed > 0 && totalProcessing === 0 ? 'partial_failure' : 'completed';
